...
버튼 입력을 누르고 있을 때 Repeat 횟수를 측정하는 코드를 추가로 작성하였다.
...
### Input Polling Manager

#### 구축 방법

`Input Polling System`을 구축하고 이 시스템을 활용할 수 있는 매니저를 만들었다.
...
`EInputDevice`를 통해 현재 프로그램이 동작하는 디바이스 정보를 열거한다.
...
이후 `Init`을 하면서 `EInputDevice`로 디바이스 정보를 넘겨 `InputManager`를 초기화 시킨다.
...
이후 static으로 입력 처리에 대한 접근이 가능하도록 한다. 인스턴스에 대한 접근은 싱글턴으로 구현한다.
...
이후 디바이스 별로 추상 메서드에 대한 로직을 작성해서 기능이 동작하도록 하였다.

#### 테스트

아래와 같이 `System`에서 테스트를 하였다.
	...
	void System::Frame()
	{
		...
		if (Input::IsKeyDown(KeyType::A))
		{
			FZLOG_TRACE("KeyDown : {0}", KeyType::A);
		}
		if (Input::IsMouseClicked(MouseButtonType::Left))
		{
			FZLOG_TRACE("MouseDown : {0}", MouseButtonType::Left);
		}
	
		auto pos = Input::GetMousePos();
		FZLOG_TRACE("MousePos {0}, {1}", pos.first, pos.second);
	}

<center>
<img src="{{img_url}}/img_06.png" style="max-width:527px; width:100%;">
<figcaption style="margin-top:-30px;">Input Polling<i><b>(그림 6)</b></i></figcaption>
</center>
...
- `Input Polling Manager` 구현
- `FZLog`에서 `KeyType`과 `MouseButtonType`이 대응되도록 개선
